...
uint256 hashGenesisBlock(" 0x6c3d1e2ea8a605fd646cdcfd8efa6a8380a980f08ffd12d1c0e7d8884cf7ab38");//
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 16); // Kangyuanjifen: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kangyuanjifen Signed Message:\n";
...
{
    // Kangyuanjifen: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight==1)
        nSubsidy=18880000*COIN;
    else
        nSubsidy=0*COIN;
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 30*60; // Kangyuanjifen: 30min
static const int64 nTargetSpacing = 1 * 60; // Kangyuanjifen:30s
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kangyuanjifen: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kangyuanjifen: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Kangyuanjifen: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
      const char* pszTimestamp = "Created By zaobi.org .QQ:260682605 ,欢迎咨询!2016-6-20 8:00:00";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256(" 0xec0e4b8b79e2e8d04ed8fbf240129568c7a4c82ce88715c638c3523bc3ab8275"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Kangyuanjifen: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("kangyuanjifenMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("kangyuanjifenMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static kangyuanjifenMiner(CWallet *pwallet)
{
    printf("kangyuanjifenMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kangyuanjifen-miner");
    ...
    try { while(true) {
        ...
        printf("Running kangyuanjifenMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("kangyuanjifenMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&kangyuanjifenMiner, pwallet));
}
